...
struct PokemonInfoPanel: View {
    ...
    var topIndicator: some View {
        RoundedRectangle(cornerRadius: 3).frame(width: 40, height: 6)
            .opacity(0.2)
    }
    
    var pokemonDescription: some View {
        Text(model.descriptionText).font(.callout)  .foregroundColor(Color(hex: 0x666666))
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            pokemonDescription
            Divider()
            AbilityList(model: self.model, abilityModels: abilities)
        }
    ...
    struct Header: View {
        ...
        var nameSpecies: some View {
            VStack {
                ...
                Text(model.nameEN).font(.system(size: 13))
                    .fontWeight(.bold)
                    .foregroundColor(model.color)
                Text(model.genus).font(.system(size: 13))
        ...
        var verticalDivider: some View {
            Rectangle().frame(width: 1, height: 44).foregroundColor(.black).opacity(0.1)
        }
    ...
    struct AbilityList: View {
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 12) {
                Text("技能").font(.headline).fontWeight(.bold)
                if abilityModels != nil {
                    ForEach(abilityModels!) { ability in
                    Text(ability.name).font(.subheadline).foregroundColor(self.model.color)
                        Text(ability.descriptionText).font(.footnote).foregroundColor(Color(hex: 0xaaaaaa))
                        .fixedSize(horizontal: false, vertical: true)
                    }
                }
